...
import per.goweii.visualeffect.blur.rs.ScriptC_BoxBlur

class BoxBlurEffect @JvmOverloads constructor(
    context: Context,
    radius: Float = 8F
) : BlurEffect(radius) {
    ...
    private var blurScript: ScriptC_BoxBlur? = null
    ...
    override fun doEffect(input: Bitmap, output: Bitmap) {
        ...
        blurScript.forEach_boxblur_v(allocInput)

        blurScript._input = allocOutput
        blurScript._output = allocInput
        blurScript.forEach_boxblur_h(allocOutput)
    ...
    private fun prepareScripts() {
        ...
        if (blurScript == null) {
            blurScript = ScriptC_BoxBlur(renderScript)
        }
    }
